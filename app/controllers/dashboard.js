...
export default Ember.Controller.extend( {
	...
	onMessage: function( data ) {
		...
		let self = this;
		let selecteduser = this.listData[ this.listData.map( function( e ) {
				return e.who;
			} )
	...
	actions: {
		...
		sendMessage: function( value ) {
			let user = atob( this.get( 'session.session.content.authenticated.token' )
				.split( '.' )[ 1 ] )
			console.log( user )
			let message = {}
			message.msg = value;
		...
		joinRoom: function( item ) {
			let user = JSON.parse( atob( this.get( 'session.session.content.authenticated.token' )
				.split( '.' )[ 1 ] ) )
			console.log( user )
